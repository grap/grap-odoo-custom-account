...
class TestModule(TransactionCase):
    def setUp(self):
        ...
        self.move_1 = self.env.ref("grap_account_export_ebp.move_1")
        self.sale_account = self.env.ref("grap_account_export_ebp.sale_account")
        self.receivable_account = self.env.ref(
            "grap_account_export_ebp.receivable_account"
        )
    ...
    def test_01_export_move_correct(self):
        ...
        suffix_wizard = self.WizardResPartnerAddSuffix.with_context(
            active_ids=[self.move_invoice.partner_id.id]
        ).create({})
    ...
    def test_02_export_content(self):
        ...
        self._asset_line_content(
            data_lines, self.move_1.journal_id.ebp_code, self.sale_account.code, 100, 0
        )
        self._asset_line_content(
            data_lines, self.move_1.journal_id.ebp_code, self.sale_account.code, 0, 20
        )
        self._asset_line_content(
            data_lines,
    ...
    def _asset_line_content(
        ...
        for data_line in data_lines:
            ...
            if credit:
                ok = ok and line_table[7] == "C" and float(line_table[6]) == credit
            if debit:
                ok = ok and line_table[7] == "D" and float(line_table[6]) == debit
            if ok:
