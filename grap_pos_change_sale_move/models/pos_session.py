...
class PosSession(models.Model):
        ...
        * _create_account_move_line
        ...
        for session in self:
            ...
            for order in session.order_ids.filtered(
                ...
                key = (order._prepare_date_sale_move_point_of_sale(),)
    ...
    def _create_move_point_of_sale(self, key, orders):
        ...
        move_vals = self._prepare_move_point_of_sale(key, orders)
        ...
        for order in orders:
            for line in order.lines:
                ...
                product_analytic_account_id = order._prepare_analytic_account(
                    line
                )
                ...
                tax_code_id = (
                    line.tax_ids and line.tax_ids[0].base_code_id.id or False
                )
        ...
        counterpart_line_vals = self._prepare_counterpart_move_line_point_of_sale(
            orders, move
        )
    ...
    @api.multi
    def _prepare_tax_move_line_point_of_sale(self, key, order_line_ids, move):
        self.ensure_one()
